...
Supongamos que se tiene una variable aleatoria $Y$ cuya distribución depende de un parámetro $\eta$ que será el predictor lineal y se relaciona con la media $\mu=\E(Y)$ mediante la función enlace $\eta=g(\mu)$, el modelo lineal generalizado con coeficientes variando se puede representar como:
...
$$\y_i =\x_i \beta_(\x_i) + \varepsilon_i$$
...
donde $\ymn$ es un vector aleatorio de respuestas observadas $(n\times 1)$,  $\tilde{\Nn}_k=\Xn^{(k)}\Nn_k$, $\Xn^{(k)}=\diag_{1\leq i\leq n}(x_i^{(k)})$ y $\Nn_k$ es la matriz de incidencia de $(n\times r)$ con el $(i,l)$-ésimo elemento igual al indicador $I({\rm \t}_{k_i} = {\rm \t}_{k_{l}}^0)$, con ${\rm t}_{k_{l}}^{0}$ que denota los valores distintos y ordenados de la variable explicativa ${\rm t}_{k_i}$ y $\betan_k=(\psi_{k_1},\ldots,\psi_{k_r})^{\top}$ es un $(r_k\times 1)$ vector
un vector de parámetros con $\psi_{k_l}=\beta_k(t^{0}_{k_l})$, para $l=1,\ldots,r_k$.


## Estimación

El modelo (\ref{modelo.general}), es general para la mayoría de las aplicaciones, ya que no se imponen restricciones a las funciones de los coeficientes. La estimación no paramétrica sin restricciones de estas funciones probablemente no sería posible, salvo en el caso de diseños especiales. En el caso de los datos observacionales, es probable que se vea un valor diferente para cada $\t_k$, en cada muestra. Por esta razón, se impone restricciones a las funciones de los coeficientes variando, por ejemplo, a trozos con forma paramétrica conocida o funciones suaves pero no paramétricas. Una aproximación sería a través de bases paramétricas como funciones polinómicas o trigonométricas. Normalmente, éstas no proporcionan suficiente flexibilidad y adaptabilidad local, por eso es preferible un conjunto de bases splines.    
Se procederá igual que con un modelo lineal, sólo que con varias variables definidas por los productos de cada $\x_i^{(k)}$, y las bases de $\beta_k(\t_{k_i})$ con este enfoque, las herramientas inferenciales estándar sirven para evaluar conjuntos de coeficientes, puntos de influencia, etc.  
Las características de las curvas ajustadas pueden ser diferentes con pequeños cambios en las posiciones de los nudos, sobre todo si sólo se tienen pocos.   
...
### Función Score Penalizada

Considerando $\lambda$ fijo la función de Score penalizada esta dada por:
...
\frac{\partial\ell_p(\tetn,\lambda)}{\partial \sigma^2}&=\frac{\partial}{\partial\sigma^2}\left[-\frac{n}{2}log\;\sigma^2 -\frac{1}{2\sigma^2} \left(\ymn-\sum_{j=1}^s\tilde{\Nn}_j\betan_j\right)^{\top}\left(\ymn-\sum_{j=1}^s\tilde{\Nn}_j\betan_j\right) \right]\\
    &=-\frac{n}{2\sigma^2}+ \frac{1}{\sigma^4} \left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)^{\top}\left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)
\end{align}
...
\left[-\frac{n}{2\sigma^2}+ \frac{1}{\sigma^4} \left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)^{\top}\left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)\right]\\
  &=\frac{n}{2\sigma^4}- \frac{1}{\sigma^6} \left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)^{\top}\left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)
\end{align}
...
\frac{n}{2\sigma^4}- \frac{1}{\sigma^6} \left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)^{\top}\left(\ymn- \sum_{j=1}^s\tilde{\Nn}_j\betan_j \right)\right\}\\\nonumber
  &=\frac{n}{2\sigma^4}
\end{align}
